import {makeRoom,ready,expire,guess,giveUp,leave,rematch,view} from '../public/game.js';
...
if(url.pathname.startsWith('/api/social/'))return await socialRoute(request,storage,auth);
...
const match=url.pathname.match(/^\/api\/rooms\/([A-F0-9]{12})(?:\/(join|ready|guess|giveup|leave|rematch|info|sync))?$/);if(!match)return json({error:'not_found'},404);const [,code,action]=match;if((action&&!['info','sync'].includes(action)&&request.method!=='POST')||((!action||['info','sync'].includes(action))&&request.method!=='GET'))return json({error:'method'},405);
if(action==='sync'){
  const since=Math.max(-1,Number(url.searchParams.get('since'))||0),until=Date.now()+12000;
  while(true){
    const row=await storage.prepare('SELECT data,revision,expires FROM rooms WHERE code=?').bind(code).first(),stamp=Date.now();
    if(!row||row.expires<=stamp)return json({error:'room_expired'},404);
    const g=JSON.parse(row.data),p=g.players.findIndex(x=>x?.auth===auth);if(p<0)return json({error:'room_access'},403);
    if(row.revision>since||stamp>=until)return json({code,revision:row.revision,...view(g,p,stamp)});
...
if(['ready','guess','giveup','leave','rematch'].includes(action)&&body.match!==g.match)return json({error:'stale_match',room:{code,revision:row.revision,...view(g,p,now)}},409);
changed=expire(g,now)||changed;
try{if(action==='ready'){ready(g,p,now);changed=true;}else if(action==='guess'){if(g.status==='playing'){guess(g,p,body.word,allowed[g.settings.lang],now);changed=true;}else if(!changed)return json({error:'not_playing'},409);}else if(action==='giveup'){giveUp(g,p);changed=true;}else if(action==='leave'){leave(g,p);changed=true;}else if(action==='rematch'){rematch(g,p,answer(g.settings.lang,g.answer));if(g.players.every(x=>x?.ready))ready(g,p,now);changed=true;}}catch(e){return json({error:e.message},400);}
