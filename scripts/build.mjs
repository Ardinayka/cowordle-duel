...
const assets={};for(const file of await fs.readdir('public')){if(!/\.(html|css|js|json|svg)$/.test(file))continue;assets['/'+file]={body:await fs.readFile('public/'+file,'utf8'),type:file.endsWith('.html')?'text/html; charset=utf-8':file.endsWith('.css')?'text/css; charset=utf-8':file.endsWith('.js')?'text/javascript; charset=utf-8':file.endsWith('.svg')?'image/svg+xml':'application/json'};}
